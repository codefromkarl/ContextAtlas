/**
 * PrepareHandoff Application Layer
 *
 * CLI 和 MCP adapter 统一通过此入口调用 checkpoint handoff 业务逻辑。
 */
...
export async function executePrepareHandoff(
...
function buildFeatureMemoryContextBlock(memory: FeatureMemory): ContextBlock {
  return {
    id: `memory:${normalizeName(memory.name)}` as const,
    type: 'module-summary',
    title: memory.name,
...
function buildDecisionContextBlock(decision: DecisionRecord): ContextBlock {
  const governanceState = decision.owner
    ...
    : 'unowned';
  return {
    id: `decision:${decision.id}` as const,
...
function buildRecentFindingsBlock(checkpoint: TaskCheckpoint): ContextBlock | null {
  ...
  return {
    id: `checkpoint-findings:${checkpoint.id}` as const,
...
function buildExploredRefsBlock(checkpoint: TaskCheckpoint): ContextBlock | null {
  ...
  return {
    id: `checkpoint-evidence:${checkpoint.id}` as const,
    type: 'code-evidence',
...
async function resolveReferencedContextBlocks(
  ...
  for (const blockId of [...checkpoint.activeBlockIds, ...(checkpoint.supportingRefs || [])]) {
    if (blockId.startsWith('memory:')) {
      const memory = featureByBlockId.get(blockId as `memory:${string}`);
      if (!memory) {
    ...
    if (blockId.startsWith('feature:')) {
      const memory = featureByAliasBlockId.get(blockId as `feature:${string}`);
      if (!memory) {
        unresolvedBlockIds.push(blockId);
        continue;
      }
