...
- `SearchService` 负责组织检索主流程，具体的向量/词法召回、RRF 融合、rerank 策略与片段提取分别下沉到 `HybridRecallEngine`、`RerankPolicy` 和 `SnippetExtractor`
...
这部分能力由 [MemoryStore.ts](../src/memory/MemoryStore.ts) 这个 facade 和 `memory/` 下的子模块共同承载；初始化与兼容导入由 `MemoryStoreBootstrap` 处理，主存储是 `~/.contextatlas/memory-hub.db`。
