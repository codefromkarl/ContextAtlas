...
export class Indexer {
  ...
  async init(): Promise<void> {
    this.vectorStore = await getVectorStore(this.projectId, this.vectorDim, this.snapshotId);
  }
  ...
  async indexFiles(
    ...
    if (toIndex.length > 0) {
      ...
      stats.skipped += indexResult.skipped;
    }
  ...
  private async batchIndex(
    ...
    const embeddingConfig = this.embeddingClient.getConfig();
    logger.info(
      {
        count: allTexts.length,
        files: files.length,
        batchSize: embeddingConfig.batchSize,
        maxConcurrency: embeddingConfig.maxConcurrency,
      },
      '开始批量 Embedding',
    );
    ...
    try {
      ...
      if (err instanceof EmbeddingPayloadTooLargeError) {
        const located = locatePayloadTooLargeFile(files, globalIndexByFileChunk, err.failedGlobalIndex);
        if (located) {
          batchUpdateVectorIndexHash(db, [{ path: located.offending.path, hash: located.offending.hash }]);
          logger.warn(
            {
              path: located.offending.path,
              failedGlobalIndex: err.failedGlobalIndex,
              textLength: err.textLength,
            },
            '检测到单文件触发 Embedding 413，已跳过并标记为已收敛',
          );

          const retryResult = await this.batchIndex(db, located.remaining, onProgress);
          return {
            success: retryResult.success,
            errors: retryResult.errors,
            skipped: retryResult.skipped + 1,
          };
        }
      }
      ...
      return { success: 0, errors: files.length, skipped: 0 };
    }
    ...
    if (filesToUpsert.length > 0) {
      try {
        ...
        logger.info(
          { files: filesToUpsert.length, chunks: totalChunkRecords },
          'LanceDB 批量写入完成',
        );
      } catch (err) {
        const error = err as { message?: string; stack?: string };
    ...
    // LanceDB 写入后可释放 embedding 内存，降低 FTS 阶段峰值
    embeddings = [];
    allTexts.length = 0;

    // ===== 阶段 5: 批量更新 FTS 索引 =====
    if (isChunksFtsInitialized(db) && totalChunkRecords > 0) {
      try {
        // 批量删除旧 FTS 记录
        const pathsToDelete = filesToUpsert.map((f) => f.path);
        batchDeleteFileChunksFts(db, pathsToDelete);

        // 分批插入新 FTS 记录，避免一次性构造超大数组
        const FTS_BATCH_SIZE = 2000;
        const ftsBuffer: Array<{
          chunkId: string;
          filePath: string;
          chunkIndex: number;
          breadcrumb: string;
        ...
        const flushFtsBuffer = (): void => {
          ...
          if (processedChunks >= nextLogAt || processedChunks === totalChunkRecords) {
            logger.info(
              { progress: `${processedChunks}/${totalChunkRecords}`, files: pathsToDelete.length },
              'FTS 批量更新进度',
            );
            while (nextLogAt <= processedChunks) {
              nextLogAt += 10000;
            }
          }
        };
        ...
        for (const file of filesToUpsert) {
          for (const record of file.records) {
            ...
            if (ftsBuffer.length >= FTS_BATCH_SIZE) {
              flushFtsBuffer();
            }
          }
        }
        ...
        logger.info(
          { files: pathsToDelete.length, chunks: totalChunkRecords },
          'FTS 批量更新完成',
        );
      } catch (err) {
        const error = err as { message?: string; stack?: string };
    ...
    return { success: successFiles.length, errors: errorFiles.length, skipped: 0 };
  }
