 ...
 * - 生产环境 (NODE_ENV === "production"): 加载 ~/.contextatlas/.env
 *
...
import { resolveConfigEnvCandidates } from './runtimePaths.js';
...
export const isMcpMode = hasMcpArg || isImplicitStdioMcp;

function loadEnv(): void {
  // 可能的 .env 文件路径（按优先级排序）
  const candidates = resolveConfigEnvCandidates(isDev);
...
export function getEmbeddingConfig(): EmbeddingConfig {
  ...
  const batchSize = parseInt(process.env.EMBEDDINGS_BATCH_SIZE || '20', 10);
