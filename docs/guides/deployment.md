...
ContextAtlas 提供两种互斥的接入模式，`setup:local` 会根据 `--mode` 参数决定写入哪些配置文件：

| 你的场景 | 命令 |
|---------|------|
| 只在终端用（CLI + skill） | `contextatlas setup:local --mode cli-skill` |
| 需要 MCP 客户端（Claude Desktop / Cursor / Gemini 等） | `contextatlas setup:local --mode mcp` |

**两种模式的配置文件严格互斥：**

- `cli-skill` 只写 prompt 文档和 `contextatlas-cli` skill，不写任何 MCP 客户端配置。
- `mcp` 只写 MCP 客户端配置和 `contextatlas-mcp` skill，不写 CLI skill。

> 旧版 `setup:local` 不带 `--mode` 会同时写 MCP + CLI 配置，新版要求显式选 `--mode`，无 mode 时会报错退出。

### 从旧版混搭迁移

如果你之前使用过不带 `--mode` 的 `contextatlas setup:local`，本地可能同时存在两种模式的配置文件。迁移步骤：

1. 确定你需要的模式，重新运行 `contextatlas setup:local --mode <cli-skill|mcp>`。
...
如果你希望把本地 CLI 助手（Claude Code / Codex / Gemini 等）的接入配置也一起配好，可以直接运行：

```bash
contextatlas setup:local --mode cli-skill
```

它会幂等创建或更新以下文件（仅 cli-skill 模式）：

- `~/.contextatlas/.env`
- `~/.claude/CLAUDE.md` 或 `~/Claude.md`
- `~/.codex/AGENTS.md` 或 `~/.codex/AGENT.md`
- `~/.gemini/GEMINI.md`
...
```bash
contextatlas setup:local --mode cli-skill --dry-run
pnpm setup:local -- --mode cli-skill -- --dry-run
```
...
> **推荐方式**：使用 `contextatlas setup:local --mode mcp` 自动写入 MCP 配置。如果你只需要只读检索工具（不含索引、记忆写入等），可以加 `--toolset retrieval-only`：`contextatlas setup:local --mode mcp --toolset retrieval-only`。以下手动配置方式仍然有效。
