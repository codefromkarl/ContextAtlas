...
export class SourceAdapter {
  ...
  public slice(start: number, end: number): string {
    const safeStartBoundary = this.alignStartBoundary(start);
    const safeEndBoundary = this.alignEndBoundary(end);
    if (safeEndBoundary <= safeStartBoundary) {
      return '';
    }

    if (this.domain === 'utf16' || this.domain === 'unknown') {
      // 直接使用 JS 字符串切片
      return this.code.slice(safeStartBoundary, safeEndBoundary);
    }
    ...
    const charStart = this.alignStartBoundary(this.byteToChar(safeStartBoundary));
    const charEnd = this.alignEndBoundary(this.byteToChar(safeEndBoundary));
  ...
  /**
   * 对齐切片起始边界，避免落在代理对中间（低位代理）
   */
  private alignStartBoundary(index: number): number {
    const clamped = Math.max(0, Math.min(this.code.length, index));
    if (
      clamped > 0 &&
      clamped < this.code.length &&
      this.isLowSurrogate(this.code.charCodeAt(clamped)) &&
      this.isHighSurrogate(this.code.charCodeAt(clamped - 1))
    ) {
      return clamped - 1;
    }
    return clamped;
  ...
  /**
   * 对齐切片结束边界，避免落在代理对中间（高位代理后）
   */
  private alignEndBoundary(index: number): number {
    const clamped = Math.max(0, Math.min(this.code.length, index));
    if (
      clamped > 0 &&
      clamped < this.code.length &&
  ...
  private isHighSurrogate(codePoint: number): boolean {
    return codePoint >= 0xd800 && codePoint <= 0xdbff;
  }

  private isLowSurrogate(codePoint: number): boolean {
