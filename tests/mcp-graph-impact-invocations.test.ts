...
import { handleGraphImpact } from '../src/mcp/tools/codeGraph.ts';
import { scan } from '../src/scanner/index.ts';

function makeTempRepo(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'cw-mcp-graph-impact-invocations-'));
}

test('graph_impact JSON 输出包含 invocation 调用点列表', async () => {
  ...
  const response = await handleGraphImpact(
    { symbol: 'updatePassword', direction: 'downstream', max_depth: 2, format: 'json' },
    rootPath,
  );
