...
import type { PackStats, ScoredChunk, SearchConfig, Segment } from './types.js';

export interface PackResult {
  files: Array<{ filePath: string; segments: Segment[] }>;
  stats: PackStats;
}
...
export class ContextPacker {
  ...
  async pack(chunks: ScoredChunk[]): Promise<Array<{ filePath: string; segments: Segment[] }>> {
    const result = await this.packWithStats(chunks);
    return result.files;
  }

  async packWithStats(chunks: ScoredChunk[]): Promise<PackResult> {
    if (chunks.length === 0) {
      return {
        files: [],
        stats: {
    ...
    // 2. 每个文件内合并区间 + 从原文件切片
    const db = initDb(this.projectId, this.snapshotId);
    ...
    for (const { filePath, chunks: fileChunks } of sortedFiles) {
      ...
      if (budgetedSegments.length > 0) {
        segmentCount += budgetedSegments.length;
