...
| 模块 | 职责 | 关键实现 |
|------|------|---------|
| `application/` | 业务逻辑层 | CLI 和 MCP adapter 统一入口，承载查询编排、结果整形、写入策略 |
...
| `mcp/` | MCP 协议层 | thin adapter（Zod schema + 委托调用），业务逻辑在 application 层 |
| `cli/` | CLI 命令层 | 参数解析 + 输出格式化，直接调用 application 层 |
...
上层 agent 默认通过 32 个 MCP 工具调用检索、记忆和索引能力；如设置 `CONTEXTATLAS_MCP_TOOLSET=retrieval-only`，则会收缩为 12 个只读检索、图谱、契约和记忆读取工具。
