...
/** 带用量的 Rerank 结果 */
export interface RerankDetailedResult<T = unknown> {
  results: RerankedDocument<T>[];
  usage?: RerankUsage;
}
...
export class RerankerClient {
  ...
  async rerank(
    ...
    const detailed = await this.rerankDetailed(query, documents, options);
  ...
  async rerankDetailed(
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        const usage: RerankUsage | undefined =
          data.meta?.billed_units?.search_units !== undefined || data.meta?.tokens?.input_tokens !== undefined
            ? {
                billedSearchUnits: data.meta?.billed_units?.search_units,
  ...
  async rerankWithData<T>(
    ...
    const detailed = await this.rerankWithDataDetailed(query, items, textExtractor, options);
    return detailed.results;
  }
  ...
  async rerankWithDataDetailed<T>(
    ...
    const detailed = await this.rerankDetailed(query, texts, options);

    // 附加原始数据
    return {
      results: detailed.results.map((result) => ({
        ...result,
